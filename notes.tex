...
\begin{definition}
    ...
    A $*$-homomorphism $\pi \colon \As \to \Bs(H)$ is called a representation 
...
\[ \ind{T\xi,\eta} = \ind{\xi, T^*\eta} \quad \text{for all } \xi \in H \text{ and }\eta \in H'\]
\end{definition}
\begin{proposition}
...
\begin{observation}
    A cyclic vector is obviously non-degenerate. Moreover a irreducible representation
is cyclic. Indeed, let $\pi \in \Rep(\As, H)$ be irreducible, and let $\xi$ be \ul{any}
non-zero vector in $H$. 
Then $[\pi(\As)\xi]$ is $\pi$-rnvariant, since $\pi(A)\pi(B) \xi = \pi(AB)$ (continuity argument).
so it has to be equal to $H$ since $[\pi(\As)\xi]\neq 0$. If $H\neq \{0\}$, then the assertion
is trivial (since $\pi$ is the $0$-representation of $\{0\}$). This shows that any non-zero vector is cyclic. 
...
\begin{proof}[Proof of proposition]
    We may assume $H\neq \{0\}$. Let 
\[ \Fs = \{ F\subset H : [\pi(A)\xi] \perp [\pi(A)\eta], ~\forall \xi,\eta \in F, \xi\neq \eta\} \]
ordered by inclusion. If $\Gs$ is a totally ordered subset of $\Fs$. Then 
$U_{G \in \Gs} \subseteq H$ is an upper bound for $\Gs$, so we can apply 
...
$H_j$. It s then clear that each $\pi_j$ is cuclic with cyclic vector $\xi_j$.
\end{proof}
...
(We are going to see that this resutl has a converse)
\end{proposition}
\begin{proof}
   We have 
...
(i.e. $[\cdot,\cdot]$ is like an inner-product, ecept that 
...
assosiated with $\ind{\cdot,\cdot}$. So $H_0$ is a Banach space and we may 
...
Not that if $\xi,\eta\in H_0$ the we may choose $\xi_n=\xi$ and $\eta_n =\eta$ for all
...
as the norm on $H$ obtained by completing $H$.

\end{observation}

\begin{theorem}[The GNS construction]
    Let $\vphi$ be a psotive linear functional on a $C^*$-algebra $\As$. Then 
...
\end{theorem}
\begin{proof}
    For $A,B \in \As$, set $[A,B]_{\vphi} = \vphi(B^*AA)$. Then 
...
Then we have
\begin{align*}
    ||\pi_0(A)(\dB)||_0 &= ||\dot{\ol{AB}}||_{0}^{2} = \vphi((AB)^*(AB)) 
   = \vphi(B^*A^*AB) \\
...
So $\pi_0(A)$ is bounded with $||\pi_0(A)||^2 \leq ||A||$. Since $H_0$
is dense in $H_{\vphi}$ we can extend in a unique way to $\pi_{\vphi}(A) \in \Bs(H_{\vphi})$
satisfying $||\pi(A)|| = ||\pi_0(A)||$.

One can now check that $A \mapsto \pi_{\vphi} (A)$ is a representation on $\As$
on $H_{\vphi}$:
...
\dA$.  So we get that $\left[[\pi_{\vphi}(A)](\xi_{\vphi})\right]  = [H_0] =
...
Again we have $[\pi(A)\xi_{\vphi}] = \ol{H_0}^{||\cdot||} = H_{\vphi}$.

Finnaly we have
\begin{align*}
    \vphi(A) = \vphi(e^{*}_{\al}A) = \lim_{\al} \indi{\dA,\dot{\eal}} = \indi{\dA, \xi_{\vphi}}
= \indi{\pi_{\vphi}(a)\xi_{\al},\xi_{\vphi}} \quad\text{ for all }A \in \As.
\end{align*}
